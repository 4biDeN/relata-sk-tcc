    ...
    <q-page class="q-pa-md column">
        ...
        <q-scroll-area class="q-px-sm" style="height: calc(100vh - 140px)"> <q-list bordered separator>
                <template v-if="rows.length">
                    <q-item v-for="r in rows" :key="String(r.id)" clickable @click="abrirNotificacao(r)" :class="[
                        'q-py-sm cursor-pointer',
                        r.status !== 'read' ? 'bg-green-1' : ''
                    ]">
                        ...
                        <q-item-section>
                            <div class="row items-center no-wrap q-gutter-xs">
                                ...
                                <q-badge v-if="r.canal" outline color="grey-7" :label="r.canal" class="q-ml-xs" />
                                <q-badge v-if="r.ocorrencia_id" outline color="green-7" :label="`#${r.ocorrencia_id}`"
                                    class="q-ml-xs" />
                            </div>
...
function parseOcorrenciaId(n) {
    ...
    const m = String(url).match(/\/(?:admin\/)?ocorrencias\/(\d+)/i);
...
function goAdminOcorrencia(id) {
    ...
    router.push(`/admin/ocorrencias/${Number(id)}`);
}
