            ...
            <q-step :name="1" title="Detalhes" caption="Detalhes da Ocorrência" icon="description" :done="step > 1">
                <q-card flat bordered class="section-card q-pa-md">
                    <div class="row q-col-gutter-md">
                        <div class="col-12">
                            <q-input v-model="form.ocorrencia_titulo" label="Título" dense color="green-9"
                                :rules="[v => !!v || 'Informe o título']" @update:model-value="marcarEdit" />
                        </div>

                        <div class="col-12">
                            <q-input v-model="form.ocorrencia_descricao" type="textarea" label="Descrição" autogrow
                                dense color="green-9" :rules="[v => !!v || 'Informe a descrição']"
                                @update:model-value="marcarEdit" />
                        </div>

                        <div class="col-12">
                            <q-file v-model="files" multiple max-files="6" accept="image/*"
                                label="Imagens da ocorrência (opcional)" use-chips counter dense outlined
                                color="green-9">
                                <template #append>
                            ...
                            <div class="row q-col-gutter-sm q-mt-sm">
                                <div v-for="(img, i) in previews" :key="i" class="col-6 col-sm-4 col-md-3">
                                    <q-card flat bordered class="thumb-card">
                        ...
                        <div class="col-12 col-sm-6">
                            <q-select v-model="form.ocorrencia_prioridade" :options="optsPrioridade" label="Prioridade"
                                dense color="green-9" emit-value map-options @update:model-value="marcarEdit"
                                outlined />
                        </div>

                        <div class="col-12 col-sm-6 flex items-center">
                            <q-toggle v-model="form.ocorrencia_anonima" label="Registrar como anônima" color="green-9"
                                @update:model-value="marcarEdit" />
                        </div>
                    </div>
                </q-card>
                ...
                <div class="row q-mt-md">
                    <q-space />
                    <q-btn color="green-9" label="Próximo" :disable="!detalhesOk" @click="step = 2" unelevated />
                </div>
            </q-step>

            <q-step :name="2" title="Localização" caption="Selecione no mapa" icon="place" :done="step > 2">
                <q-card flat bordered class="section-card q-pa-md">
                    <div class="map-section">
                        <div ref="mapEl" :class="['leaflet-map', isMobile ? 'h-280' : 'h-420']"></div>

                        <div class="row items-center q-gutter-sm q-mt-sm">
                            <q-btn outline dense icon="my_location" label="Usar minha localização" color="green-9"
                                @click="usarGeolocalizacao" :loading="gettingLocation" />
                            <q-btn flat dense icon="refresh" label="Limpar seleção" color="green-9"
                                @click="resetarLocal" :disable="estado === 'idle'" />
                            <q-space />
                            <q-badge v-if="estado === 'idle'" class="soft-badge">Selecione um ponto no mapa</q-badge>
                            <q-badge v-else-if="estado === 'picked'" color="green-7"
                                class="soft-badge text-white">Endereço
                                sugerido</q-badge>
                            <q-badge v-else-if="estado === 'edited'" color="orange-7"
                                class="soft-badge text-white">Endereço
                                ajustado</q-badge>
                            <q-spinner-dots v-if="loadingGeocode" size="18px" color="green-9" />
                        </div>
                    </div>
                </q-card>

                <div class="row q-mt-md">
                    <q-btn flat label="Voltar" color="green-9" @click="step = 1" />
                    <q-space />
                    <q-btn color="green-9" label="Próximo" :disable="!coordsOk" @click="step = 3" unelevated />
                </div>
            </q-step>

            <q-step :name="3" title="Endereço" caption="Confirme ou edite" icon="home_pin">
                <q-card flat bordered class="section-card q-pa-md">
                    <div class="row q-col-gutter-md">
                        <div class="col-12 col-sm-3">
                            <q-input v-model="form.local.local_estado" label="UF" dense maxlength="2" color="green-9"
                                @update:model-value="marcarEdit" />
                        </div>

                        <div class="col-12 col-sm-9">
                            <q-select v-model="form.local.local_municipio_id" v-model:input-value="municipioInput"
                                label="Município" dense color="green-9" use-input clearable emit-value map-options
                                input-debounce="300" :options="municipioOptions" :loading="municipioLoading"
                        ...
                        <div class="col-12 col-sm-6">
                            <q-input v-model="form.local.local_rua" label="Logradouro" dense color="green-9"
                                :rules="[v => !!String(v || '').trim() || 'Informe o logradouro']"
                                @update:model-value="marcarEdit" />
                        </div>

                        <div class="col-12">
                            <q-input v-model="form.local.local_complemento" label="Complemento" dense color="green-9"
                                @update:model-value="marcarEdit" />
                        </div>

                        <div class="col-12 col-sm-6">
                            <q-input v-model.number="form.local.local_latitude" label="Latitude" type="number" dense
                                color="green-9" :rules="[v => v !== null || 'Informe a latitude']"
                                @update:model-value="marcarEdit" />
                        </div>

                        <div class="col-12 col-sm-6">
                            <q-input v-model.number="form.local.local_longitude" label="Longitude" type="number" dense
                                color="green-9" :rules="[v => v !== null || 'Informe a longitude']"
                                @update:model-value="marcarEdit" />
                        </div>
                    </div>
                </q-card>
                ...
                <div class="row q-mt-md">
                    <q-btn flat label="Voltar" color="green-9" @click="step = 2" />
                    <q-space />
                    <q-btn color="green-9" :loading="creating" :disable="!podeSalvar" label="Enviar" @click="salvar"
                        unelevated />
                </div>
            </q-step>
        </q-stepper>
...
async function setMunicipioFromGeocode(cidade, uf) {
    await carregarMunicipios(cidade)
    const nCidade = normalize(cidade)
    const nUf = normalize(uf)
    let opt =
        municipioOptions.value.find(o => normalize(o.label) === normalize(`${cidade} - ${uf || ''}`)) ||
        municipioOptions.value.find(o => normalize(o.raw?.nome) === nCidade && (!uf || normalize(o.raw?.uf) === nUf)) ||
        municipioOptions.value.find(o => normalize(o.raw?.nome).startsWith(nCidade))
    if (opt) {
        suppressEdit = true
        form.local.local_municipio_id = opt.value
        municipioInput.value = opt.label
        form.local.local_municipio_nome = opt.raw.nome
        if (!form.local.local_estado && opt.raw.uf) form.local.local_estado = opt.raw.uf
        await nextTick()
        suppressEdit = false
        estado.value = 'picked'
        return true
    ...
    try {
        ...
        if (cidade) {
            ...
            if (!form.local.local_municipio_id) {
                const ok = await setMunicipioFromGeocode(cidade, form.local.local_estado)
                if (!ok) municipioInput.value = cidade
            }
        }
...
async function salvar() {
    ...
    form.ocorrencia_user_id = Number(localStorage.getItem('user_id'))
    ...
    try {
        ...
        await store.criarOcorrencia(fd, true)
...
.stepper-elev {
    border-radius: 14px;
    background: #fff;
    border: 1px solid rgba(13, 71, 161, 0);
}
