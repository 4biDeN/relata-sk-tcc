<template>
    <q-page padding>
        <q-table :rows="filteredRows" :columns="columns" :visible-columns="visibleColumns" row-key="ocorrencia_id"
            :loading="loading" :pagination="{ rowsPerPage: 10 }" :rows-per-page-options="[10, 20, 50, 100]"
            :row-class="rowClass" @row-click="irDetalhe" class="cursor-pointer ocorrencias-table"
            :grid="$q.screen.lt.md" card-class="q-pa-sm card--mobile-full" card-container-class="grid-mobile--full">
            <template #top>
                <div class="filters-wrap">
                    <div class="filters">
                        <q-input v-model="busca" dense placeholder="Buscar por título ou protocolo"
                            @keyup.enter="carregar" />
                        <q-select v-model="statusSel" :options="statusOptions" label="Status" dense emit-value
            ...
            <template #item="props">
                <div class="q-pa-sm q-card q-card--bordered q-hoverable card--mobile-full"
                    @click="irDetalhe(null, props.row)">
                    <div class="row no-wrap items-start">
                        ...
                        <div class="column ellipsis">
                            <div class="row items-center no-wrap">
                                <div class="text-subtitle2 text-weight-medium ellipsis">{{ props.row.ocorrencia_titulo
                                    }}</div>
                            ...
                            <div class="row items-center q-mt-xs q-gutter-xs">
                                ...
                                <q-chip dense square :color="props.row.atrib_color" text-color="white">
                                    {{ props.row.atrib_label }}
                                </q-chip>
                                <q-chip size="sm" :color="props.row.anon_bool ? 'grey-7' : 'positive'"
                                    text-color="white" outline>
                                    {{ props.row.anon_bool ? 'Anônima' : 'Identificada' }}
                                </q-chip>
                            </div>

                            <div class="text-caption text-grey-6 q-mt-xs">
                                {{ props.row.ocorrencia_data_fmt }}
                            </div>
                        </div>
                    </div>
                </div>
...
const visibleColumns = computed(() => {
    return $q.screen.lt.md
        ? ['thumb', 'ocorrencia_titulo', 'ocorrencia_status_nome']
...
@media (max-width: 900px) {
    ...
    .ocorrencias-table :deep(.q-table__grid-content.grid-mobile--full) {
        padding-left: 0 !important;
        padding-right: 0 !important;
    }

    .ocorrencias-table :deep(.q-table__grid-content.grid-mobile--full .q-table__grid-item) {
        padding-left: 0 !important;
        padding-right: 0 !important;
    ...
    .ocorrencias-table :deep(.card--mobile-full) {
        width: 100% !important;
        max-width: 100% !important;
