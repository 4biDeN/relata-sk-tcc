...
const updateOcorrencia = async (req, res) => {
    try {
        ...
        if (updateData.ocorrencia_status !== undefined) {
            updateData.ocorrencia_status = Number(updateData.ocorrencia_status);
        }
        if (updateData.ocorrencia_prioridade !== undefined) {
            updateData.ocorrencia_prioridade = Number(updateData.ocorrencia_prioridade);
        }
        if (updateData.ocorrencia_atribuida !== undefined) {
            updateData.ocorrencia_atribuida = Number(updateData.ocorrencia_atribuida);
        }
        if (updateData.ocorrencia_anonima !== undefined) {
            const v = updateData.ocorrencia_anonima;
            updateData.ocorrencia_anonima =
                typeof v === "boolean" ? v : String(v).toLowerCase() === "true";
        }

        if (
            updateData.ocorrencia_prioridade !== undefined &&
            ![1, 2, 3, 4].includes(updateData.ocorrencia_prioridade)
        ) {
            return res.status(422).json({ message: "ocorrencia_prioridade inválida" });
        }

        if (
            updateData.ocorrencia_status !== undefined &&
            ![1, 2, 3, 4, 5].includes(updateData.ocorrencia_status)
        ) {
            return res.status(422).json({ message: "ocorrencia_status inválido" });
        }

        const userId = req.user.sub;
        const updated = await ocorrenciaService.updateOcorrencia(id, updateData, { userId });
